...
def main():
    ...
    qr_server = [
        b
        for a in QR_SERVERS_GROUPS
        for b in a["qr_servers"]
        if b["server_name"] == server_name
    ][0]
    ...
    curl_string = (
        ...
        f"platform/online/api/list?moduleName=front.orders&className=ru.edgex.quickresto.modules.front.orders.OrderInfo"
    )
    result = subprocess.run(curl_string.split(), capture_output=True)
    print(f"[curl string]:\n{curl_string}\n")
    # print(f'[cur args]:\n{result.args}\n')
    # print(f'[out]:\n{result.stdout[:500]}')
